...
async def test_priorities_hide_completed_lists(client, ensure_db):
    ...
    r = await client.post(f'/html_no_js/lists/{vid}/priority', data={'priority': '5', '_csrf': csrf}, follow_redirects=False)
    assert r.status_code in (302, 303)
    r2 = await client.post(f'/html_no_js/lists/{cid}/priority', data={'priority': '6', '_csrf': csrf}, follow_redirects=False)
    ...
    # 1) Default behavior: cookie absent => hide_completed True => completed list should NOT appear
    ...
    # 2) Explicit show completed via cookie value '0' => completed list should appear
    r5 = await client.get('/html_no_js/priorities', cookies={'priorities_hide_completed': '0'})
    ...
    r6 = await client.get('/html_no_js/priorities', cookies={'priorities_hide_completed': '1'})
