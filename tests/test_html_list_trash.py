...
from app.models import ListState, ListTrashMeta, Todo
...
async def test_list_delete_moves_to_trash_and_restore(client: AsyncClient):
    ...
    async with async_session() as sess:
        ...
        q3 = await sess.exec(select(ListTrashMeta).where(ListTrashMeta.list_id == sub.id))
        meta = q3.first()
    ...
    # restore sublist
    resp = await client.post(f'/html_no_js/trash/lists/{sub.id}/restore', data={'_csrf': csrf}, follow_redirects=False)
    assert resp.status_code in (303,302,200)
...
async def test_permanent_delete_list_from_trash(client: AsyncClient):
    ...
    # move to trash
    resp = await client.post(f'/html_no_js/lists/{lst['id']}/delete', data={'_csrf': csrf})
    assert resp.status_code in (303,302,200)

    # permanently delete from trash
    resp = await client.post(f'/html_no_js/trash/lists/{lst['id']}/delete', data={'_csrf': csrf}, follow_redirects=False)
    assert resp.status_code in (303,302,200)
    ...
    async with async_session() as sess:
        q = await sess.exec(select(Tombstone).where(Tombstone.item_type == 'list').where(Tombstone.item_id == lst['id']))
        ts = q.first()
