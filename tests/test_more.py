...
async def test_move_update_delete_todo(client):
    ...
    r2 = await client.post(
        "/todos",
        json={"text": "task1", "note": "initial", "list_id": dlist['id']},
    )
    ...
    r4 = await client.patch(
        f"/todos/{tid}",
        json={"text": "task1 updated", "note": "changed"},
    )
...
async def test_multiple_completion_types_behavior(client):
    ...
    r = await client.post(
        "/todos",
        json={"text": "multi complete", "list_id": ml['id']},
    )
    assert r.status_code == 200
    todo = r.json()
    tid = todo["id"]
    ...
    r3 = await client.post(
        f"/todos/{tid}/complete",
        params={"completion_type": "review", "done": True},
    )
    assert r3.status_code == 200
    ...
    r4 = await client.post(
        f"/todos/{tid}/complete",
        params={"completion_type": "default", "done": False},
    )
    assert r4.status_code == 200
    ...
    # Support both dict shape {name: bool} and list-of-dicts
    # [{completion_type_id, done}]
