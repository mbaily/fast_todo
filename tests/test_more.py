...
async def test_move_update_delete_todo(client):
    ...
    dlist = rdef.json()
    r2 = await client.post("/todos", params={"text": "task1", "note": "initial", "list_id": dlist['id']})
    ...
    # move to work list
    r3 = await client.patch(f"/todos/{tid}", params={"list_id": work_id})
    assert r3.status_code == 200
    moved = r3.json()
    assert moved["list_id"] == work_id

    # update text and note
    r4 = await client.patch(f"/todos/{tid}", params={"text": "task1 updated", "note": "changed"})
...
async def test_multiple_completion_types_behavior(client):
    ...
    ml = rlist.json()
    r = await client.post("/todos", params={"text": "multi complete", "list_id": ml['id']})
    assert r.status_code == 200
    todo = r.json()
    tid = todo["id"]
    ...
    assert r5.status_code == 200
    got = r5.json()
    comps = {c["completion_type_id"]: c["done"] for c in got["completions"]}
    ...
    assert any(c["done"] is False for c in got["completions"]) 
    assert any(c["done"] is True for c in got["completions"]) 
...
async def test_long_unicode_text(client):
    ...
    l = rlist.json()
    r = await client.post("/todos", params={"text": long_text, 'list_id': l['id']})
    assert r.status_code == 200
    todo = r.json()
