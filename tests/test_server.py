...
async def test_create_and_get_todo(client):
    ...
    lst = rl.json()
    r = await client.post("/todos", params={"text": "buy milk", "list_id": lst['id']})
...
async def test_defer_and_undefer(client):
    ...
    lst = rl.json()
    r = await client.post("/todos", params={"text": "defer me", "list_id": lst['id']})
    todo = r.json()
    todo_id = todo["id"]
...
async def test_completion_types_and_mark_done(client):
    ...
    lst = rl.json()
    r = await client.post("/todos", params={"text": "complete me", "list_id": lst['id']})
    todo = r.json()
    todo_id = todo["id"]
    ...
    got = r3.json()
    assert any(c["done"] for c in got["completions"]) 
