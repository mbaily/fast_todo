#!/usr/bin/env python3
"""
CDP micro-driver: Click the unignore button on a previously found event.
Reads event details from .cdp_event.json

Usage: python scripts/cdp_unignore_event.py
"""
import asyncio
import json
import os
...
async def main():
    ...
    print(f"🎯 Unignoring event: {event['title']}")
    ...
    async with async_playwright() as p:
        ...
        # Find and click the unignore button
        unignore_btn = todo.locator('button.occ-unignore').first()
        
        if await unignore_btn.count() == 0:
            print(f"⚠️  No unignore button found - event may not be ignored")
            await browser.close()
            exit(1)
        
        print(f"\n🖱️  Clicking unignore button...")
        await unignore_btn.click()
        ...
        # Check for response
        response_logs = [log for log in console_logs if 'unignore' in log.lower() and 'response' in log]
        print(f"   Server responses: {len(response_logs)}")
        
        print(f"\n✅ Unignore button clicked")
